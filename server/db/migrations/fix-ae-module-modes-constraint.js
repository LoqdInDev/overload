...
async function runMigration() {
  ...
  const defaultWs = await db.prepare(
  ...
  if (wsId) {
    await db.prepare('UPDATE ae_module_modes SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
  }
  ...
  await db.exec('ALTER TABLE ae_module_modes_new RENAME TO ae_module_modes');

  // Also backfill workspace_id on approval queue, action log, rules, notifications, settings
  if (wsId) {
    await db.prepare('UPDATE ae_approval_queue SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
    await db.prepare('UPDATE ae_action_log SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
    await db.prepare('UPDATE ae_rules SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
    await db.prepare('UPDATE ae_notifications SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
    await db.prepare('UPDATE ae_settings SET workspace_id = ? WHERE workspace_id IS NULL').run(wsId);
  }
