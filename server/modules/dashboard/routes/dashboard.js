...
async function getRouter() {
  ...
  router.get('/summary', async (req, res) => {
    ...
    const weekActivities = safeGet(
      "SELECT COUNT(*) as count FROM activity_log WHERE workspace_id = ? AND created_at >= NOW() - INTERVAL '7 days'", [wsId]
    )?.count || 0;
    const prevWeekActivities = safeGet(
      "SELECT COUNT(*) as count FROM activity_log WHERE workspace_id = ? AND created_at >= NOW() - INTERVAL '14 days' AND created_at < NOW() - INTERVAL '7 days'", [wsId]
    )?.count || 0;
