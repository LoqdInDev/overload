...
router.get('/:id', (req, res) => {
  try {
    const item = db.prepare('SELECT * FROM sc_tickets WHERE id = ?').get(req.params.id);
...
router.post('/', (req, res) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO sc_tickets (subject, description, customer_email, priority, status, assigned_to, ai_response) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(subject, description || null, customer_email || null, priority || 'medium', status || 'open', assigned_to || null, ai_response || null);
    const item = db.prepare('SELECT * FROM sc_tickets WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', (req, res) => {
  try {
    const existing = db.prepare('SELECT * FROM sc_tickets WHERE id = ?').get(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Not found' });

    const { subject, description, customer_email, priority, status, assigned_to, ai_response } = req.body;
    db.prepare(
      'UPDATE sc_tickets SET subject = ?, description = ?, customer_email = ?, priority = ?, status = ?, assigned_to = ?, ai_response = ?, updated_at = datetime(\'now\') WHERE id = ?'
    ).run(
    ...
    const updated = db.prepare('SELECT * FROM sc_tickets WHERE id = ?').get(req.params.id);
    res.json(updated);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/:id', (req, res) => {
  try {
    const existing = db.prepare('SELECT * FROM sc_tickets WHERE id = ?').get(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Not found' });
    db.prepare('DELETE FROM sc_tickets WHERE id = ?').run(req.params.id);
...
router.get('/templates/list', (req, res) => {
  try {
    const templates = db.prepare('SELECT * FROM sc_templates ORDER BY created_at DESC').all();
    res.json(templates);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/templates', (req, res) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO sc_templates (name, category, content) VALUES (?, ?, ?)'
    ).run(name, category || null, content || null);
    const item = db.prepare('SELECT * FROM sc_templates WHERE id = ?').get(result.lastInsertRowid);
    res.status(201).json(item);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/templates/:id', (req, res) => {
  try {
    db.prepare('DELETE FROM sc_templates WHERE id = ?').run(req.params.id);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
