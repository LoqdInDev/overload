...
router.get('/:id', async (req, res) => {
  try {
    const wsId = req.workspace.id;
    const item = await db.prepare('SELECT * FROM sc_tickets WHERE id = ? AND workspace_id = ?').get(req.params.id, wsId);
...
router.put('/:id', async (req, res) => {
  try {
    const wsId = req.workspace.id;
    const existing = await db.prepare('SELECT * FROM sc_tickets WHERE id = ? AND workspace_id = ?').get(req.params.id, wsId);
    ...
    const updated = await db.prepare('SELECT * FROM sc_tickets WHERE id = ? AND workspace_id = ?').get(req.params.id, wsId);
    res.json(updated);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const existing = await db.prepare('SELECT * FROM sc_tickets WHERE id = ? AND workspace_id = ?').get(req.params.id, wsId);
    if (!existing) return res.status(404).json({ error: 'Not found' });
    await db.prepare('DELETE FROM sc_tickets WHERE id = ? AND workspace_id = ?').run(req.params.id, wsId);
...
router.get('/templates/list', async (req, res) => {
  try {
    const wsId = req.workspace.id;
    const templates = await db.prepare('SELECT * FROM sc_templates WHERE workspace_id = ? ORDER BY created_at DESC').all(wsId);
    res.json(templates);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.delete('/templates/:id', async (req, res) => {
  try {
    const wsId = req.workspace.id;
    await db.prepare('DELETE FROM sc_templates WHERE id = ? AND workspace_id = ?').run(req.params.id, wsId);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
