...
router.post('/generate', async (req, res) => {
  ...
  try {
    ...
    await logActivity('budget-optimizer', 'generate', `Generated ${type || 'content'}`, 'AI generation', null, wsId);
...
router.get('/budgets', async (req, res) => {
  const wsId = req.workspace.id;
  try {
    const budgets = await db.prepare('SELECT * FROM bo_budgets WHERE workspace_id = ? ORDER BY created_at DESC').all(wsId);
...
router.post('/budgets', async (req, res) => {
  ...
  try {
    ...
    const budget = await db.prepare('SELECT * FROM bo_budgets WHERE id = ? AND workspace_id = ?').get(result.lastInsertRowid, wsId);
...
router.get('/budgets/:id', async (req, res) => {
  const wsId = req.workspace.id;
  try {
    const budget = await db.prepare('SELECT * FROM bo_budgets WHERE id = ? AND workspace_id = ?').get(req.params.id, wsId);
    if (!budget) {
      return res.status(404).json({ success: false, error: 'Budget not found' });
    }
    const allocations = await db.prepare('SELECT * FROM bo_allocations WHERE budget_id = ? AND workspace_id = ? ORDER BY created_at ASC').all(req.params.id, wsId);
...
router.delete('/budgets/:id', async (req, res) => {
  const wsId = req.workspace.id;
  try {
    await db.prepare('DELETE FROM bo_allocations WHERE budget_id = ? AND workspace_id = ?').run(req.params.id, wsId);
    await db.prepare('DELETE FROM bo_budgets WHERE id = ? AND workspace_id = ?').run(req.params.id, wsId);
...
Only return JSON.`);
    ...
    await logActivity('budget-optimizer', 'optimize', 'Generated budget optimization', goal || 'Sales', null, wsId);
  } catch (err) {
