...
export default function ApiManagerPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6 animate-fade-in"><p className="hud-label mb-2" style={{ color: '#0ea5e9' }}>API MANAGER</p><h1 className="text-2xl font-bold text-white mb-1">API Manager</h1><p className="text-sm text-gray-500">Manage API keys, monitor usage, and review request logs</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
        {[{ l: 'ACTIVE KEYS', v: '3' }, { l: 'TOTAL REQUESTS', v: '12.4K' }, { l: 'AVG RESPONSE', v: '120ms' }, { l: 'RATE LIMIT USAGE', v: '34%' }].map((s, i) => (<div key={i} className="panel rounded-xl p-4"><p className="hud-label mb-1">{s.l}</p><p className="text-2xl font-bold text-white font-mono">{s.v}</p></div>))}
      </div>
      ...
      {tab === 'keys' && (<div className="space-y-2 animate-fade-in">{MOCK_KEYS.map(k => (<div key={k.id} className="panel rounded-xl p-4 flex items-center gap-4"><div className="w-9 h-9 rounded-lg bg-sky-500/10 flex items-center justify-center flex-shrink-0"><svg className="w-4 h-4 text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" /></svg></div><div className="flex-1 min-w-0"><p className="text-sm font-semibold text-gray-200">{k.name}</p><p className="text-[10px] text-gray-500 font-mono mt-0.5">{k.key}</p></div><div className="text-right"><p className="text-xs font-bold text-sky-400 font-mono">{k.requests}</p><p className="text-[10px] text-gray-500">requests</p></div><span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{k.status}</span></div>))}</div>)}
      {tab === 'logs' && (<div className="panel rounded-xl overflow-hidden animate-fade-in"><div className="divide-y divide-indigo-500/[0.04]"><div className="flex items-center gap-3 px-4 py-2 text-[10px] text-gray-500 font-semibold uppercase"><span className="w-14">Method</span><span className="flex-1">Endpoint</span><span className="w-12 text-right">Status</span><span className="w-16 text-right">Time</span><span className="w-20 text-right">When</span></div>{MOCK_LOGS.map((l, i) => (<div key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.01] transition-colors"><span className={`w-14 text-[10px] font-bold px-1.5 py-0.5 rounded text-center ${methodColors[l.method]}`}>{l.method}</span><span className="flex-1 text-xs text-gray-300 font-mono truncate">{l.endpoint}</span><span className={`w-12 text-right text-xs font-bold font-mono ${statusColor(l.status)}`}>{l.status}</span><span className="w-16 text-right text-[10px] text-gray-500 font-mono">{l.time}</span><span className="w-20 text-right text-[10px] text-gray-500">{l.timestamp}</span></div>))}</div></div>)}
      {tab === 'ai-tools' && (<div className="space-y-4 animate-fade-in"><div className="grid grid-cols-2 gap-2">{AI_TEMPLATES.map(t => (<button key={t.name} onClick={() => generate(t)} disabled={generating} className={`panel-interactive rounded-xl p-4 text-left ${selectedTemplate?.name === t.name ? 'border-sky-500/20' : ''}`}><p className="text-xs font-bold text-gray-300">{t.name}</p><p className="text-[10px] text-gray-600 mt-1 line-clamp-2">{t.prompt}</p></button>))}</div>{(generating || output) && <div className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-sky-400 animate-pulse' : 'bg-sky-400'}`} /><span className="hud-label" style={{ color: '#38bdf8' }}>{generating ? 'GENERATING...' : 'READY'}</span></div><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-sky-400 ml-0.5 animate-pulse" />}</pre></div>}</div>)}
    </div>
  );
}
