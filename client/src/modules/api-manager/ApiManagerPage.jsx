...
export default function ApiManagerPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10">
      ...
      {tab === 'keys' && (<div className="space-y-3 animate-fade-in">{MOCK_KEYS.map(k => (<div key={k.id} className="panel rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6"><div className="flex items-center gap-3 sm:gap-6"><div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-sky-500/10 flex items-center justify-center flex-shrink-0"><svg className="w-5 h-5 text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" /></svg></div><div className="flex-1 min-w-0"><p className="text-base font-semibold text-gray-200">{k.name}</p><p className="text-xs text-gray-500 font-mono mt-0.5 truncate">{k.key}</p></div></div><div className="flex flex-wrap items-center gap-3 sm:gap-6 sm:ml-auto"><div className="text-left sm:text-right"><p className="text-sm font-bold text-sky-400 font-mono">{k.requests}</p><p className="text-xs text-gray-500">requests</p></div><span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{k.status}</span></div></div>))}</div>)}
      {tab === 'logs' && (<div className="panel rounded-2xl overflow-hidden animate-fade-in"><div className="overflow-x-auto"><div className="min-w-[480px] divide-y divide-indigo-500/[0.04]"><div className="flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-3 text-xs text-gray-500 font-semibold uppercase"><span className="w-14">Method</span><span className="flex-1">Endpoint</span><span className="w-12 text-right">Status</span><span className="w-16 text-right">Time</span><span className="w-20 text-right">When</span></div>{MOCK_LOGS.map((l, i) => (<div key={i} className="flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-3.5 hover:bg-white/[0.01] transition-colors"><span className={`w-14 text-[10px] font-bold px-1.5 py-0.5 rounded text-center flex-shrink-0 ${methodColors[l.method]}`}>{l.method}</span><span className="flex-1 text-sm text-gray-300 font-mono truncate">{l.endpoint}</span><span className={`w-12 text-right text-sm font-bold font-mono flex-shrink-0 ${statusColor(l.status)}`}>{l.status}</span><span className="w-16 text-right text-xs text-gray-500 font-mono flex-shrink-0">{l.time}</span><span className="w-20 text-right text-xs text-gray-500 flex-shrink-0">{l.timestamp}</span></div>))}</div></div></div>)}
