...
export default function AbTestingPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6 animate-fade-in"><p className="hud-label mb-2" style={{ color: '#f472b6' }}>A/B TESTING</p><h1 className="text-2xl font-bold text-white mb-1">A/B Test Manager</h1><p className="text-sm text-gray-500">Run experiments, track variants, and find winning combinations</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
        {[{ l: 'ACTIVE TESTS', v: '8' }, { l: 'TOTAL VARIANTS', v: '24' }, { l: 'AVG LIFT', v: '+12.3%' }, { l: 'WIN RATE', v: '67%' }].map((s, i) => (<div key={i} className="panel rounded-xl p-4"><p className="hud-label mb-1">{s.l}</p><p className="text-2xl font-bold text-white font-mono">{s.v}</p></div>))}
      </div>
      ...
      {tab === 'tests' && (<div className="space-y-2 animate-fade-in">{MOCK_TESTS.map(t => (<div key={t.id} className="panel rounded-xl p-4 flex items-center gap-4"><div className="flex-1"><p className="text-sm font-semibold text-gray-200">{t.name}</p><p className="text-[10px] text-gray-500 mt-0.5">{t.variants} variants</p></div><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${typeColors[t.type]}`}>{t.type}</span><span className="text-sm font-bold text-pink-400 font-mono">{t.lift}</span><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${statusColors[t.status]}`}>{t.status}</span></div>))}</div>)}
      {tab === 'results' && (<div className="space-y-2 animate-fade-in">{MOCK_RESULTS.map((r, i) => (<div key={i} className="panel rounded-xl p-4"><div className="flex items-center justify-between mb-2"><p className="text-sm font-semibold text-gray-200">{r.test}</p><span className="text-sm font-bold text-pink-400 font-mono">{r.lift}</span></div><div className="flex items-center gap-4 text-[10px] text-gray-500"><span>Winner: <span className="text-gray-300 font-semibold">{r.winner}</span></span><span>Confidence: <span className="text-emerald-400 font-semibold">{r.confidence}</span></span><span>Conversions: <span className="text-gray-300">{r.conversions}</span></span><span>Sample: <span className="text-gray-300">{r.sampleSize}</span></span></div></div>))}</div>)}
      {tab === 'ai-tools' && (<div className="space-y-4 animate-fade-in"><div className="grid grid-cols-2 gap-2">{AI_TEMPLATES.map(t => (<button key={t.name} onClick={() => generate(t)} disabled={generating} className={`panel-interactive rounded-xl p-4 text-left ${selectedTemplate?.name === t.name ? 'border-pink-500/20' : ''}`}><p className="text-xs font-bold text-gray-300">{t.name}</p><p className="text-[10px] text-gray-600 mt-1 line-clamp-2">{t.prompt}</p></button>))}</div>{(generating || output) && <div className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-pink-400 animate-pulse' : 'bg-pink-400'}`} /><span className="hud-label" style={{ color: '#f472b6' }}>{generating ? 'GENERATING...' : 'READY'}</span></div><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-pink-400 ml-0.5 animate-pulse" />}</pre></div>}</div>)}
    </div>
  );
}
