...
export default function AbTestingPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      {tab === 'tests' && (<div className="space-y-3 animate-fade-in">{MOCK_TESTS.map(t => (<div key={t.id} className="panel rounded-2xl p-4 sm:p-6 flex items-center gap-4 sm:gap-6"><div className="flex-1"><p className="text-base font-semibold text-gray-200">{t.name}</p><p className="text-xs text-gray-500 mt-0.5">{t.variants} variants</p></div><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${typeColors[t.type]}`}>{t.type}</span><span className="text-base font-bold text-pink-400 font-mono">{t.lift}</span><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${statusColors[t.status]}`}>{t.status}</span></div>))}</div>)}
      {tab === 'results' && (<div className="space-y-3 animate-fade-in">{MOCK_RESULTS.map((r, i) => (<div key={i} className="panel rounded-2xl p-4 sm:p-6"><div className="flex items-center justify-between mb-2"><p className="text-base font-semibold text-gray-200">{r.test}</p><span className="text-base font-bold text-pink-400 font-mono">{r.lift}</span></div><div className="flex items-center gap-4 sm:gap-6 text-xs text-gray-500"><span>Winner: <span className="text-gray-300 font-semibold">{r.winner}</span></span><span>Confidence: <span className="text-emerald-400 font-semibold">{r.confidence}</span></span><span>Conversions: <span className="text-gray-300">{r.conversions}</span></span><span>Sample: <span className="text-gray-300">{r.sampleSize}</span></span></div></div>))}</div>)}
