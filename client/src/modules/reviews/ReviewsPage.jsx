  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12 animate-fade-in">
      <div className="flex items-center gap-3 sm:gap-5 mb-6 sm:mb-8">
        ...
        <div><p className="hud-label text-[11px]" style={{ color: '#eab308' }}>{TOOLS.find(t => t.id === activeTool)?.name?.toUpperCase()}</p><h2 className="text-lg font-bold text-white">{TOOLS.find(t => t.id === activeTool)?.name}</h2></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          {activeTool === 'respond' && <div className="panel rounded-2xl p-4 sm:p-6"><p className="hud-label text-[11px] mb-3">QUICK TEMPLATES</p><div className="grid grid-cols-2 lg:grid-cols-3 gap-3">{TEMPLATES.map(t => (<button key={t.name} onClick={() => { setStars(t.stars); setReviewText(t.prompt); }} className={`text-left px-3 py-2 rounded-lg border text-xs transition-all ${reviewText === t.prompt ? 'border-yellow-500/30 bg-yellow-500/8 text-yellow-300' : 'border-indigo-500/8 text-gray-400 hover:text-gray-200'}`}><div className="flex gap-0.5 mb-1">{Array.from({ length: 5 }, (_, i) => (<span key={i} style={{ color: i < t.stars ? '#eab308' : '#374151' }}>&#9733;</span>))}</div><p className="font-semibold">{t.name}</p></button>))}</div></div>}
          ...
          {activeTool === 'respond' && <div className="panel rounded-2xl p-4 sm:p-6"><p className="hud-label text-[11px] mb-3">STAR RATING</p><div className="flex gap-3">{[1,2,3,4,5].map(s => (<button key={s} onClick={() => setStars(s)} className="text-2xl transition-transform hover:scale-110" style={{ color: s <= stars ? '#eab308' : '#374151' }}>&#9733;</button>))}</div></div>}
      ...
      {output && <div className="mt-6 animate-fade-up"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-yellow-400 animate-pulse' : 'bg-emerald-400'}`} /><span className="hud-label text-[11px]" style={{ color: generating ? '#facc15' : '#4ade80' }}>{generating ? 'GENERATING...' : 'RESPONSE READY'}</span></div><div className="panel rounded-2xl p-4 sm:p-7"><pre className="text-base text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-yellow-400 ml-0.5 animate-pulse" />}</pre></div></div>}
    </div>
  );
}
