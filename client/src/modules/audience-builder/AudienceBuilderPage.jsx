...
export default function AudienceBuilderPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6 animate-fade-in"><p className="hud-label mb-2" style={{ color: '#8b5cf6' }}>AUDIENCE BUILDER</p><h1 className="text-2xl font-bold text-white mb-1">Audience Builder</h1><p className="text-sm text-gray-500">Build, manage, and optimize audiences across platforms</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
        {[{ l: 'TOTAL AUDIENCES', v: '12' }, { l: 'TOTAL REACH', v: '2.4M' }, { l: 'SEGMENTS', v: '34' }, { l: 'LOOKALIKES', v: '8' }].map((s, i) => (<div key={i} className="panel rounded-xl p-4"><p className="hud-label mb-1">{s.l}</p><p className="text-2xl font-bold text-white font-mono">{s.v}</p></div>))}
      </div>
      ...
      {tab === 'audiences' && (<div className="space-y-2 animate-fade-in">{MOCK_AUDIENCES.map(a => (<div key={a.id} className="panel rounded-xl p-4 flex items-center gap-4"><div className="flex-1"><p className="text-sm font-semibold text-gray-200">{a.name}</p><p className="text-[10px] text-gray-500 mt-0.5">{typeLabels[a.type]} &bull; {a.size} reach</p></div><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${platformColors[a.platform]}`}>{a.platform}</span><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${a.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-gray-500/10 text-gray-500 border border-gray-500/20'}`}>{a.status}</span></div>))}</div>)}
      {tab === 'segments' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-2 animate-fade-in">{MOCK_SEGMENTS.map((s, i) => (<div key={i} className="panel rounded-xl p-4 flex items-center gap-4"><div className="flex-1"><p className="text-sm font-semibold text-gray-200">{s.name}</p><p className="text-[10px] text-gray-500 mt-0.5">{s.count} users</p></div><span className={`text-xs font-bold font-mono ${s.trend.startsWith('+') ? 'text-emerald-400' : 'text-red-400'}`}>{s.trend}</span></div>))}</div>)}
      {tab === 'ai-tools' && (<div className="space-y-4 animate-fade-in"><div className="grid grid-cols-2 gap-2">{AI_TEMPLATES.map(t => (<button key={t.name} onClick={() => generate(t)} disabled={generating} className={`panel-interactive rounded-xl p-4 text-left ${selectedTemplate?.name === t.name ? 'border-violet-500/20' : ''}`}><p className="text-xs font-bold text-gray-300">{t.name}</p><p className="text-[10px] text-gray-600 mt-1 line-clamp-2">{t.prompt}</p></button>))}</div>{(generating || output) && <div className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-violet-400 animate-pulse' : 'bg-violet-400'}`} /><span className="hud-label" style={{ color: '#a78bfa' }}>{generating ? 'GENERATING...' : 'READY'}</span></div><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-violet-400 ml-0.5 animate-pulse" />}</pre></div>}</div>)}
    </div>
  );
}
