...
export default function AffiliatesPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      {tab === 'programs' && (<div className="space-y-3 animate-fade-in">{MOCK_PROGRAMS.map(p => (<div key={p.id} className="panel rounded-2xl p-4 sm:p-6 flex items-center gap-4 sm:gap-6"><div className="flex-1"><p className="text-base font-semibold text-gray-200">{p.name}</p><p className="text-xs text-gray-500 mt-0.5">{p.type === 'percentage' ? `${p.rate}% commission` : `$${p.rate} per sale`} &bull; {p.affiliates} affiliates</p></div><span className="text-base font-bold text-emerald-400 font-mono">${p.revenue.toLocaleString()}</span><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${p.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-gray-500/10 text-gray-500 border border-gray-500/20'}`}>{p.status}</span></div>))}</div>)}
      {tab === 'affiliates' && (<div className="panel rounded-2xl overflow-hidden animate-fade-in"><div className="divide-y divide-indigo-500/[0.04]">{MOCK_AFFILIATES.map(a => (<div key={a.id} className="flex items-center gap-6 px-6 py-4 hover:bg-white/[0.01] transition-colors"><div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-sm font-bold text-emerald-400 flex-shrink-0">{a.name.split(' ').map(n => n[0]).join('')}</div><div className="flex-1 min-w-0"><p className="text-sm font-semibold text-gray-300">{a.name}</p><p className="text-xs text-gray-500 font-mono">{a.code}</p></div><div className="text-right text-xs text-gray-500"><p>{a.clicks} clicks</p><p>{a.conversions} conv.</p></div><span className="text-base font-bold text-emerald-400 font-mono">${a.revenue.toLocaleString()}</span></div>))}</div></div>)}
