    ...
    <div className="p-4 sm:p-6 lg:p-12 animate-fade-in">
      ...
      {activeTool === 'find' && (
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="panel rounded-2xl p-4 sm:p-6"><p className="hud-label text-[11px] mb-3">FOLLOWER RANGE</p><div className="grid grid-cols-4 gap-1.5">{FOLLOWER_RANGES.map(f => (<button key={f.id} onClick={() => setFollowerRange(f.id)} className={`chip text-[10px] justify-center flex-col items-center py-2 ${followerRange === f.id ? 'active' : ''}`} style={followerRange === f.id ? { background: 'rgba(236,72,153,0.15)', borderColor: 'rgba(236,72,153,0.3)', color: '#f472b6' } : {}}><span className="font-bold">{f.name}</span><span className="text-[10px] opacity-60">{f.label}</span></button>))}</div></div>
