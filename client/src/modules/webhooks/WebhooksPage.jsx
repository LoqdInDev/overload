...
export default function WebhooksPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      {tab === 'logs' && (<div className="panel rounded-2xl overflow-hidden animate-fade-in"><div className="overflow-x-auto"><div className="divide-y divide-indigo-500/[0.04] min-w-[600px]"><div className="flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-3 text-xs text-gray-500 font-semibold uppercase"><span className="flex-1">Webhook</span><span className="w-32">Event</span><span className="w-12 text-right">Status</span><span className="w-16 text-right">Latency</span><span className="w-20 text-right">When</span></div>{MOCK_LOGS.map((l, i) => (<div key={i} className="flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-3.5 hover:bg-white/[0.01] transition-colors"><span className="flex-1 text-sm font-semibold text-gray-300">{l.webhook}</span><span className="w-32 text-xs text-gray-500 font-mono">{l.event}</span><span className={`w-12 text-right text-sm font-bold font-mono ${statusColor(l.status)}`}>{l.status}</span><span className="w-16 text-right text-xs text-gray-500 font-mono">{l.latency}</span><span className="w-20 text-right text-xs text-gray-500">{l.timestamp}</span></div>))}</div></div></div>)}
