...
export default function WebhooksPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6 animate-fade-in"><p className="hud-label mb-2" style={{ color: '#f97316' }}>WEBHOOKS</p><h1 className="text-2xl font-bold text-white mb-1">Webhooks</h1><p className="text-sm text-gray-500">Manage event hooks, monitor deliveries, and track performance</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
        {[{ l: 'ACTIVE HOOKS', v: '5' }, { l: 'EVENTS TODAY', v: '89' }, { l: 'SUCCESS RATE', v: '99.2%' }, { l: 'AVG LATENCY', v: '45ms' }].map((s, i) => (<div key={i} className="panel rounded-xl p-4"><p className="hud-label mb-1">{s.l}</p><p className="text-2xl font-bold text-white font-mono">{s.v}</p></div>))}
      </div>
      ...
      {tab === 'webhooks' && (<div className="space-y-2 animate-fade-in">{MOCK_WEBHOOKS.map(w => (<div key={w.id} className="panel rounded-xl p-4"><div className="flex items-center gap-4 mb-2"><div className="flex-1"><p className="text-sm font-semibold text-gray-200">{w.name}</p><p className="text-[10px] text-gray-500 font-mono mt-0.5">{w.url}</p></div><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${w.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-gray-500/10 text-gray-500 border border-gray-500/20'}`}>{w.status}</span></div><div className="flex items-center gap-2 flex-wrap"><span className="text-[10px] text-gray-500">Last: {w.lastTriggered}</span><span className="text-[10px] text-gray-600">&bull;</span>{w.events.map((e, i) => (<span key={i} className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-orange-500/10 text-orange-400 border border-orange-500/15">{e}</span>))}</div></div>))}</div>)}
      {tab === 'logs' && (<div className="panel rounded-xl overflow-hidden animate-fade-in"><div className="divide-y divide-indigo-500/[0.04]"><div className="flex items-center gap-3 px-4 py-2 text-[10px] text-gray-500 font-semibold uppercase"><span className="flex-1">Webhook</span><span className="w-32">Event</span><span className="w-12 text-right">Status</span><span className="w-16 text-right">Latency</span><span className="w-20 text-right">When</span></div>{MOCK_LOGS.map((l, i) => (<div key={i} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.01] transition-colors"><span className="flex-1 text-xs font-semibold text-gray-300">{l.webhook}</span><span className="w-32 text-[10px] text-gray-500 font-mono">{l.event}</span><span className={`w-12 text-right text-xs font-bold font-mono ${statusColor(l.status)}`}>{l.status}</span><span className="w-16 text-right text-[10px] text-gray-500 font-mono">{l.latency}</span><span className="w-20 text-right text-[10px] text-gray-500">{l.timestamp}</span></div>))}</div></div>)}
      {tab === 'ai-tools' && (<div className="space-y-4 animate-fade-in"><div className="grid grid-cols-2 gap-2">{AI_TEMPLATES.map(t => (<button key={t.name} onClick={() => generate(t)} disabled={generating} className={`panel-interactive rounded-xl p-4 text-left ${selectedTemplate?.name === t.name ? 'border-orange-500/20' : ''}`}><p className="text-xs font-bold text-gray-300">{t.name}</p><p className="text-[10px] text-gray-600 mt-1 line-clamp-2">{t.prompt}</p></button>))}</div>{(generating || output) && <div className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-orange-400 animate-pulse' : 'bg-orange-400'}`} /><span className="hud-label" style={{ color: '#fb923c' }}>{generating ? 'GENERATING...' : 'READY'}</span></div><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-orange-400 ml-0.5 animate-pulse" />}</pre></div>}</div>)}
    </div>
  );
}
