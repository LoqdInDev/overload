...
export default function TeamPage() {
  ...
  const generate = async (template) => {
    ...
    try {
      const res = await fetch('/api/team/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'content', prompt: template.prompt }) });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5 mb-6 sm:mb-8 stagger">
        {[
          { label: 'TEAM MEMBERS', value: '5', sub: 'Across all roles' },
          { label: 'ACTIVE NOW', value: '3', sub: 'Currently online' },
          { label: 'PENDING INVITES', value: '2', sub: '1 expired' },
      ...
      {tab === 'members' && (
        <div className="animate-fade-in space-y-3">
          {MOCK_MEMBERS.map(member => (
            <div key={member.id} className="panel rounded-2xl p-4 sm:p-6 flex items-center gap-4 sm:gap-6 hover:border-indigo-500/15 transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-base font-semibold text-gray-200">{member.name}</p>
              ...
              <span className="text-xs text-gray-500 w-24 text-right hidden md:block">{member.lastActive}</span>
      ...
      {tab === 'invites' && (
        <div className="animate-fade-in space-y-4 sm:space-y-6">
          ...
          <div className="panel rounded-2xl p-4 sm:p-7">
            ...
            <div className="flex gap-3">
              <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="email@example.com"
                className="flex-1 bg-white/[0.03] border border-indigo-500/[0.08] rounded-xl px-4 py-3 text-base text-gray-200 focus:outline-none focus:border-indigo-500/30" />
              <select value={inviteRole} onChange={e => setInviteRole(e.target.value)}
                className="bg-white/[0.03] border border-indigo-500/[0.08] rounded-xl px-4 py-3 text-base text-gray-200 focus:outline-none focus:border-indigo-500/30 appearance-none w-32">
                <option value="admin">Admin</option>
                <option value="editor">Editor</option>
                <option value="viewer">Viewer</option>
              </select>
              <button className="px-5 py-3 rounded-xl text-base font-bold tracking-wide transition-all"
          ...
          <div className="panel rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-indigo-500/[0.04]">
              <p className="hud-label text-[11px]" style={{ color: MODULE_COLOR }}>PENDING INVITES</p>
            </div>
            <div className="divide-y divide-indigo-500/[0.04]">
              {MOCK_INVITES.map(invite => (
                <div key={invite.id} className="flex items-center gap-4 sm:gap-6 px-4 sm:px-6 py-4 hover:bg-white/[0.01] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-600 mt-0.5">Sent {invite.sentAt}</p>
                  </div>
