...
export default function ProductFeedsPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl mx-auto">
      <div className="mb-6 animate-fade-in"><p className="hud-label mb-2" style={{ color: '#64748b' }}>PRODUCT FEEDS</p><h1 className="text-2xl font-bold text-white mb-1">Product Feed Manager</h1></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 stagger">
        {[{ l: 'TOTAL PRODUCTS', v: MOCK_PRODUCTS.length }, { l: 'ACTIVE', v: totalActive }, { l: 'CHANNELS', v: totalChannels }, { l: 'OUT OF STOCK', v: MOCK_PRODUCTS.filter(p => p.status === 'out_of_stock').length }].map((s, i) => (
          <div key={i} className="panel rounded-xl p-4"><p className="hud-label mb-1">{s.l}</p><p className="text-2xl font-bold text-white font-mono">{s.v}</p></div>
        ))}
      </div>
      ...
      {tab === 'products' && (
        <div className="animate-fade-in">
          <div className="flex gap-3 mb-4">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..." className="input-field rounded-lg px-4 py-2 text-sm flex-1" />
          ...
          <div className="panel rounded-xl overflow-hidden">
            <div className="grid grid-cols-[1fr_80px_80px_60px_auto_80px] px-4 py-2 border-b border-indigo-500/[0.06] text-[10px] font-bold text-gray-500">
            ...
            {filtered.map(p => (
              <div key={p.id} className="grid grid-cols-[1fr_80px_80px_60px_auto_80px] items-center px-4 py-3 border-b border-indigo-500/[0.03] hover:bg-white/[0.01] transition-colors">
                <span className="text-xs font-semibold text-gray-200 truncate">{p.name}</span>
                <span className="text-[10px] text-gray-500 font-mono">{p.sku}</span>
                <span className="text-xs text-gray-300 font-mono text-right">${p.price}</span>
                <span className={`text-xs font-mono text-right ${p.stock === 0 ? 'text-red-400' : p.stock < 20 ? 'text-yellow-400' : 'text-gray-400'}`}>{p.stock}</span>
      ...
      {tab === 'channels' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 animate-fade-in stagger">
          {CHANNELS.map(c => {
            const count = MOCK_PRODUCTS.filter(p => p.channels.includes(c.id)).length;
            return (
              <div key={c.id} className="panel rounded-xl p-4">
                <div className="flex items-center gap-3 mb-3"><div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${c.color}15`, border: `1px solid ${c.color}25` }}><span className="text-xs font-bold" style={{ color: c.color }}>{c.name[0]}</span></div><div><p className="text-sm font-bold text-gray-200">{c.name}</p><p className="text-[10px] text-gray-500">{count} products synced</p></div></div>
      ...
      {tab === 'ai-optimize' && (
        <div className="space-y-4 animate-fade-in">
          <div className="grid grid-cols-2 gap-3">{AI_TOOLS.map(t => (<button key={t.name} onClick={() => generate(t)} disabled={generating} className="panel-interactive rounded-xl p-4 text-left"><p className="text-xs font-bold text-gray-200">{t.name}</p><p className="text-[10px] text-gray-500 mt-1 line-clamp-2">{t.prompt}</p></button>))}</div>
          {(generating || output) && <div className="panel rounded-xl p-5"><div className="flex items-center gap-2 mb-3"><div className={`w-2 h-2 rounded-full ${generating ? 'bg-slate-400 animate-pulse' : 'bg-emerald-400'}`} /><span className="hud-label" style={{ color: generating ? '#94a3b8' : '#4ade80' }}>{generating ? 'OPTIMIZING...' : 'OPTIMIZATION READY'}</span></div><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-slate-400 ml-0.5 animate-pulse" />}</pre></div>}
        </div>
      )}
    </div>
  );
