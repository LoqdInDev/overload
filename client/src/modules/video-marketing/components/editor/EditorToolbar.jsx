...
export default function EditorToolbar({ editor, showImport, setShowImport }) {
  ...
  return (
    <>
      <div className={`flex items-center gap-2 px-4 py-2 border-b ${border} flex-shrink-0 ${dark ? 'bg-[#050508]' : 'bg-white/60'}`}>
        {/* Import */}
        ...
        {/* Clip count */}
        <span className={`text-[11px] ${dark ? 'text-gray-500' : 'text-[#94908A]'}`}>
      ...
      {showImport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowImport(false)}>
          <div
            ...
            className={`w-full max-w-2xl max-h-[70vh] rounded-2xl border ${border} ${dark ? 'bg-[#12121a]' : 'bg-white'} flex flex-col overflow-hidden shadow-2xl`}
          >
