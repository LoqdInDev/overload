  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-5 mb-6 sm:mb-8 stagger">
        {[{ l: 'TOTAL BUDGET', v: '$45.2K' }, { l: 'ACTIVE CHANNELS', v: '6' }, { l: 'BEST ROAS', v: '4.2x' }, { l: 'BUDGET USED', v: '78%' }].map((s, i) => (<div key={i} className="panel rounded-2xl p-4 sm:p-6"><p className="hud-label text-[11px] mb-1">{s.l}</p><p className="text-xl sm:text-2xl font-bold text-white font-mono">{s.v}</p></div>))}
      </div>
      ...
      {tab === 'overview' && (<div className="space-y-4 animate-fade-in">{MOCK_ALLOCATIONS.map(a => { const pct = Math.round((a.spent / a.budget) * 100); return (<div key={a.id} className="panel rounded-2xl p-4 sm:p-6"><div className="flex items-center justify-between mb-2"><p className="text-base font-semibold text-gray-200">{a.channel}</p><span className="text-base font-bold font-mono" style={{ color: '#34d399' }}>{a.roas}x ROAS</span></div><div className="w-full h-1.5 rounded-full bg-white/5 mb-1.5"><div className="h-full rounded-full" style={{ width: `${pct}%`, background: pct > 90 ? '#f87171' : '#059669' }} /></div><div className="flex justify-between text-xs text-gray-500"><span>${a.spent.toLocaleString()} / ${a.budget.toLocaleString()}</span><span>{pct}% used</span></div></div>); })}</div>)}
      {tab === 'allocations' && (<div className="panel rounded-2xl overflow-hidden animate-fade-in"><div className="overflow-x-auto"><div className="divide-y divide-indigo-500/[0.04] min-w-[480px]"><div className="flex items-center gap-4 sm:gap-6 px-4 sm:px-6 py-3 text-xs text-gray-500 font-semibold uppercase"><span className="flex-1">Channel</span><span className="w-20 text-right">Budget</span><span className="w-20 text-right">Spent</span><span className="w-16 text-right">ROAS</span><span className="w-16 text-right">Share</span></div>{MOCK_ALLOCATIONS.map(a => (<div key={a.id} className="flex items-center gap-4 sm:gap-6 px-4 sm:px-6 py-4 hover:bg-white/[0.01] transition-colors"><span className="flex-1 text-sm font-semibold text-gray-300">{a.channel}</span><span className="w-20 text-right text-sm text-gray-400 font-mono">${a.budget.toLocaleString()}</span><span className="w-20 text-right text-sm text-gray-400 font-mono">${a.spent.toLocaleString()}</span><span className="w-16 text-right text-sm font-bold font-mono" style={{ color: '#34d399' }}>{a.roas}x</span><span className="w-16 text-right text-sm text-gray-500 font-mono">{Math.round((a.budget / totalBudget) * 100)}%</span></div>))}<div className="flex items-center gap-4 sm:gap-6 px-4 sm:px-6 py-4 border-t border-indigo-500/10"><span className="flex-1 text-sm font-bold text-gray-200">Total</span><span className="w-20 text-right text-sm font-bold text-gray-200 font-mono">${totalBudget.toLocaleString()}</span><span className="w-20 text-right text-sm font-bold text-gray-200 font-mono">${totalSpent.toLocaleString()}</span><span className="w-16" /><span className="w-16 text-right text-sm font-bold text-gray-200 font-mono">100%</span></div></div></div></div>)}
