...
export default function BudgetOptimizerPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-12">
      ...
      {tab === 'allocations' && (<div className="panel rounded-2xl overflow-hidden animate-fade-in"><div className="divide-y divide-indigo-500/[0.04]"><div className="flex items-center gap-4 sm:gap-6 px-6 py-3 text-xs text-gray-500 font-semibold uppercase"><span className="flex-1">Channel</span><span className="w-20 text-right">Budget</span><span className="w-20 text-right">Spent</span><span className="w-16 text-right">ROAS</span><span className="w-16 text-right">Share</span></div>{MOCK_ALLOCATIONS.map(a => (<div key={a.id} className="flex items-center gap-4 sm:gap-6 px-6 py-4 hover:bg-white/[0.01] transition-colors"><span className="flex-1 text-sm font-semibold text-gray-300">{a.channel}</span><span className="w-20 text-right text-sm text-gray-400 font-mono">${a.budget.toLocaleString()}</span><span className="w-20 text-right text-sm text-gray-400 font-mono">${a.spent.toLocaleString()}</span><span className="w-16 text-right text-sm font-bold font-mono" style={{ color: '#34d399' }}>{a.roas}x</span><span className="w-16 text-right text-sm text-gray-500 font-mono">{Math.round((a.budget / totalBudget) * 100)}%</span></div>))}<div className="flex items-center gap-4 sm:gap-6 px-6 py-4 border-t border-indigo-500/10"><span className="flex-1 text-sm font-bold text-gray-200">Total</span><span className="w-20 text-right text-sm font-bold text-gray-200 font-mono">${totalBudget.toLocaleString()}</span><span className="w-20 text-right text-sm font-bold text-gray-200 font-mono">${totalSpent.toLocaleString()}</span><span className="w-16" /><span className="w-16 text-right text-sm font-bold text-gray-200 font-mono">100%</span></div></div></div>)}
