  ...
  if (!activeTool) return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-8 animate-fade-in">
        <p className="hud-label mb-2" style={{ color: '#14b8a6' }}>SEO SUITE</p>
        <h1 className="text-2xl font-bold text-white mb-1">Choose an SEO tool</h1>
        <p className="text-sm text-gray-500">AI-powered SEO analysis, keyword research, and content optimization</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 stagger">
        {TOOLS.map(t => (
          <button key={t.id} onClick={() => setActiveTool(t.id)} className="panel-interactive rounded-xl p-5 text-center group">
            <div className="w-10 h-10 rounded-lg mx-auto mb-3 flex items-center justify-center" style={{ background: 'rgba(20,184,166,0.1)', border: '1px solid rgba(20,184,166,0.12)' }}>
              <svg className="w-5 h-5 text-teal-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d={t.icon} /></svg>
            </div>
      ...
      <div className="mt-10">
        <div className="flex items-center gap-3 mb-4"><p className="hud-label">QUICK START</p><div className="flex-1 hud-line" /></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 stagger">
          {Object.entries(TEMPLATES).flatMap(([tool, tmpls]) => tmpls.slice(0, 1).map(t => (
            <button key={`${tool}-${t.name}`} onClick={() => { setActiveTool(tool); setPrompt(t.prompt); }} className="panel-interactive rounded-lg p-4 text-left group">
              <p className="hud-label mb-1" style={{ color: '#14b8a6' }}>{TOOLS.find(x => x.id === tool)?.name}</p>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><p className="hud-label" style={{ color: '#14b8a6' }}>{tool?.name?.toUpperCase()}</p><h2 className="text-lg font-bold text-white">{tool?.name}</h2></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <div className="panel rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {templates.map(t => (
                <button key={t.name} onClick={() => setPrompt(t.prompt)} className={`text-left px-3 py-2.5 rounded-lg border text-xs transition-all ${prompt === t.prompt ? 'border-teal-500/30 bg-teal-500/8 text-teal-300' : 'border-indigo-500/8 bg-white/[0.01] text-gray-400 hover:text-gray-200'}`}>
                  <p className="font-semibold">{t.name}</p><p className="text-[10px] text-gray-600 mt-0.5 line-clamp-1">{t.prompt}</p>
                </button>
              ))}
            </div>
          </div>
          <div className="panel rounded-xl p-4">
            <p className="hud-label mb-3">{activeTool === 'optimize' ? 'PASTE CONTENT' : 'DESCRIBE YOUR QUERY'}</p>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={activeTool === 'optimize' ? 6 : 3} placeholder={activeTool === 'optimize' ? 'Paste your content here for SEO analysis...' : 'Enter your topic, URL, or keyword...'} className="w-full input-field rounded-lg px-4 py-3 text-sm resize-none" />
          </div>
        ...
        <div className="space-y-4">
          <div className="panel rounded-xl p-4">
            <p className="hud-label mb-3">TARGET COUNTRY</p>
          ...
          <div className="panel rounded-xl p-4">
            <p className="hud-label mb-3">SEARCH INTENT</p>
      ...
      {output && (
        <div className="mt-4 animate-fade-up">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-teal-400" /><span className="hud-label" style={{ color: '#2dd4bf' }}>{generating ? 'ANALYZING...' : 'RESULTS'}</span></div>
          ...
          <div className="panel rounded-xl p-5"><pre className="text-sm text-gray-300 whitespace-pre-wrap font-sans leading-relaxed">{output}{generating && <span className="inline-block w-1.5 h-4 bg-teal-400 ml-0.5 animate-pulse" />}</pre></div>
        </div>
      )}
    </div>
  );
